...
from leaf.torchmetrics.sixd import Projection2d, ADD, Cmd, ADDAUC, PoseCompose, MeanTranslationError, MeanRotationError
...
def test_pose_compose(model, model_diameter, pose_list, K):
    ...
    for k, v in pose_metric.summarize().items():
        assert torch.isnan(torch.tensor(v)) or v == 0
